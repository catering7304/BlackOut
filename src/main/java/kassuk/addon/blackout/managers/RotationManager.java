    ...
    private void onRender(Render3DEvent event) {
        ...
        if (timer > 0 && target != null && lastDir != null) {
            if (SettingUtils.shouldVanillaRotate()) {
                mc.player.setYaw(MathHelper.lerpAngleDegrees(mc.getRenderTickCounter().getTickDelta(false), prevDir[0], currentDir[0]));
                mc.player.setPitch(MathHelper.lerp(mc.getRenderTickCounter().getTickDelta(false), prevDir[1], currentDir[1]));
            }
        } else if (target != null) {
            target = null;
            priority = 1000;
