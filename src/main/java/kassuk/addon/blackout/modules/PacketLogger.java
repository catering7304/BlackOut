...
// TODO if (packet instanceof CustomPayloadC2SPacket p) return "CustomPayload data:" + p.getData() + " channel:" + p.payload().id();
        ...
        if (packet instanceof CommandExecutionC2SPacket p) return "CommandExecution command:" + p.command() + " salt:" + p.command();
        if (packet instanceof CraftRequestC2SPacket p) return "CraftRequest syncId:" + p.getSyncId() + " craftAll:" + p.shouldCraftAll() + " recipe:" + p.getRecipeId();
        ...
        if (packet instanceof Full p) return "PlayerMove.Full x:" + p.getX(0) + " y:" + p.getY(0) + " z:" + p.getZ(0) + " yaw:" + p.getYaw(0) + " pitch:" + p.getPitch(0) + " ground:" + p.isOnGround();
        if (packet instanceof PositionAndOnGround p) return "PlayerMove.Pos x:" + p.getX(0) + " y:" + p.getY(0) + " z:" + p.getZ(0) + " ground:" + p.isOnGround();
        if (packet instanceof LookAndOnGround p) return "PlayerMove.Look yaw:" + p.getYaw(0) + " pitch:" + p.getPitch(0) + " ground:" + p.isOnGround();
        if (packet instanceof OnGroundOnly p) return "PlayerMove.Ground ground:" + p.isOnGround();
        ...
        if (packet instanceof UpdateBeaconC2SPacket p) return "UpdateBeacon primary:" + (p.primary().isPresent() ? p.primary().get() : "null") + (p.secondary().isPresent() ? p.secondary().get() : "null");
        ...
        if (packet instanceof CustomPayloadS2CPacket p) return "CustomPayload channel:" + p.payload().getId();
        ...
        if (packet instanceof ReadyS2CPacket p) return "Ready state:" + (p.getPacketId() == null ? "null" : p.getPacketId().id());
        ...
        if (packet instanceof EnterReconfigurationS2CPacket p) return "EnterReconfiguration state:" + (p.getPacketId() == null ? "null" : p.getPacketId().id());
        if (packet instanceof EntitiesDestroyS2CPacket p) return "EntitiesDestroy size:" + p.getEntityIds().size();
        if (packet instanceof EntityAnimationS2CPacket p) return "EntityAnimation id:" + p.getEntityId() + " animationId:" + p.getAnimationId();
        ...
        if (packet instanceof EntityStatusEffectS2CPacket p) return "EntityStatusEffect entityId:" + p.getEntityId() + " effect:" + p.getEffectId().getIdAsString() + " amplifier:" + p.getAmplifier() + " duration:" + p.getDuration() + " showIcon:" + p.shouldShowIcon() + " particles:" + p.shouldShowParticles() + " ambient:" + p.isAmbient();
        ...
        if (packet instanceof RemoveEntityStatusEffectS2CPacket p) return "RemoveEntityStatusEffect effect:" + (p.effect() == null ? "null" : p.effect().getIdAsString());
        ...
        if (packet instanceof ChangeUnlockedRecipesS2CPacket p) return "UnlockRecipes action:" + p.getAction().name();
    ...
    private String offers(TradeOfferList offers) {
        ...
        for (TradeOffer o : offers)
            builder.append(o.copySellItem().getItem().getName()).append(" -> ").append(o.getOriginalFirstBuyItem().getItem().getName()).append(" & ").append(o.getSecondBuyItem().get().itemStack().getName()).append("\n");
        return builder.toString();
    }
